...
- Plugin list: `lua/plugin-specs.lua` (`spec = "plugin-specs"` in lazy setup). Per-plugin setup in `lua/plugins-configs/<name>.lua` via the local `get_config(name)` helper. Post-load overrides in `after/plugin/<name>.lua`.
